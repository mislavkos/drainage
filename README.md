...
- **Or trace the canyon instead of tapping it.** Press **✏ Draw the canyon**, tap along the route from the top down to where you walk out, then press **Done** (Enter also works; Escape cancels). The drainage is taken at your *last* point. This matters because a drainage is only ever computed at one point: tap where you drop in and every tributary joining below you is missing from the answer, so the number you read is smaller than the water you will be standing in. The traced line stays on the map as a check — the app tells you if part of your route falls outside the shaded drainage, which is how a line traced the wrong way round (bottom to top, pour point at the wrong end) announces itself. The line is local to your device: share links still carry the pour point only.
