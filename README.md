...
- **Or import the canyon's KML / GPX.** Press **Import KML / GPX** (or drag the file onto the page) and pick which section is the descent — ropewiki, CalTopo and Gaia exports all work, and both formats give the same answer. The app measures each section against the USGS 3DEP elevation model and pre-selects the steepest real descent, so the label reads `Class 3 Section — 808 ft, ↓ 138 ft (17%)` and the choice stays obvious even when a track is called `Track 003`. It ranks by drop but ignores anything shallower than 8% — a car shuttle or access road descends further than the canyon it serves, and gradient is what tells them apart (validated across 178 canyons: that one rule cut the mis-picks from 14% to 8%). Where no section is steep enough to be obviously the canyon, it asks you instead of guessing. The drainage is taken at that section's low end. Note it deliberately does **not** take the route's lowest point: a canyon's exit walk often drops below the canyon's own outflow on its way to the car, and for Keyhole that reads 19× too big. Ropewiki links can't be fetched directly — the site is behind a bot challenge and sends no CORS headers — so download the file and drop it in.
- **Or trace the canyon instead of tapping it.** Press **✏ Draw canyon ↓**, tap down the canyon from the top to the bottom of the descent — not the hike back out — then press **Done** (Enter also works; Escape cancels). The drainage is taken at your *last* point. This matters because a drainage is only ever computed at one point: tap where you drop in and every tributary joining below you is missing from the answer, so the number you read is smaller than the water you will be standing in. The traced line stays on the map as a check — the app tells you if part of your route falls outside the shaded drainage, which is how a line traced the wrong way round (bottom to top, pour point at the wrong end) announces itself. The line is local to your device: share links still carry the pour point only.
- **Or paste a coordinate — or a whole link.** The box under those two buttons takes a coordinate in any of the forms below, and it also takes a share link someone sent you: paste the whole thing, including a multi-pin `#pins:` link, and it behaves exactly as if you had opened it (a pin list still asks before it saves anything). That is the way in when the app is installed to your home screen, where there is no address bar to paste a link into.
...
- Basic anonymized usage counting is on by default and can be switched off in the app's "About this app" section. It records that a lookup, export, share, pin, embed load, or click through from an embed happened, when it happened, the domain name of the page that linked to or embedded the app (never its path), your screen size, and, for a lookup only, which US state. Nothing else, and never a coordinate.
