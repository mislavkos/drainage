...
- **The bottom end is the LAST vertex, chosen by trace direction, not by elevation.** You trace a canyon the way you walk it. Comparing the two ends' *drainage areas* (the downstream one is bigger by definition) is the theoretically correct pick and was rejected: it needs two full delineations before the answer, inside the app's documented main bug factory, to decide something the user already told us by tracing downhill. Picking the lower end by *elevation* was re-examined 2026-09-07 and still rejected, though the original reason for it (a new flaky service) is spent — the import path already depends on 3DEP. The reasons that stand are specific to a hand-traced line: (1) **the walk out often climbs**, and the route leaving the basin on an ascending exit is normal and documented below, so "lower endpoint" can select the canyon's TOP — the understating direction; (2) **the import path is safe only because it filters by gradient first**, and on an 11–23% descent the sign of the drop is not in doubt. A traced line has no such filter, so on the gentle slots (Orderville, Zebra) a tap landing on a bank instead of the channel can flip the sign. The failure mode to avoid is not "the user is warned", it is "the app silently picks the top".
- **The containment check is the whole reason to have a line.** Everything upstream of the pour point is inside the basin by construction, so a vertex OUTSIDE it means the snap found the wrong channel, or the line was traced bottom-to-top — which puts the pour point at the TOP and understates the drainage. Turf point-in-polygon per vertex in `renderBasinInfo()` catches both, and the majority-outside case names the backwards trace explicitly, because that is the dangerous one. The minority case is a `note`, not a `warn` — a route that leaves the drainage on the walk out is a real and correct thing to see.
- **Two vertices are discounted from the count, and getting this wrong silently disabled the whole check.** The pour vertex is excluded by index (`pourIdx()`), because it sits ON the boundary and reads either way — 6 m from the edge on a measured real basin. Anything merely *just* outside is excluded by distance (>100 m via `pointToLineDistance`), because the TOP of a CORRECTLY traced canyon sits near the drainage divide, and metres of slop there is not a reversal — a real one puts the far vertex a whole canyon length out (2238 m measured). The original code instead absorbed the pour vertex with a blanket `out > 1` gate, which meant a **2-point trace could never warn at all**: one of any two vertices is the pour point, so at most one can be genuinely outside. Two taps, top and bottom, is the simplest trace anyone draws, and it had no check whatsoever — field-reported 2026-09-07 on a real reversal that understated the drainage by 17%. Pinned by tests in both directions; do not re-tighten the gate to paper over a false positive.
- **The warning offers the flip, because the polyline is not what is wrong.** Only the end poured at is wrong, so `renderBasinInfo()` appends a "Use the other end" button that calls `useRoute(pts, otherEnd(pts))` — same line, other end. Re-entering Draw calls `clearRoute()` and discards the trace, so "just trace it again" costs the user every vertex. The button is on the loud branch ONLY: on the quiet branch a few points out is the normal walk out, and flipping there would break a correct answer. `pourIdx()` finds the live end by coordinate rather than assuming the last vertex, because an imported segment can already pour at `coords[0]` when 3DEP called that end lower — assuming would make the button a silent no-op on exactly those files. The route array is captured in the closure, since a later map tap nulls `route`.
- **The warning says one thing, because a warning nobody reads is not a warning.** 251 characters down to 77, field-flagged 2026-09-07 as too much to read. It names the evidence and then asks the one question the reader can actually answer — the line runs outside, did you draw it bottom to top? — and the button does the rest. The consequence ("the real drainage would be bigger") was cut too: it is the reason the check exists, but a reader who answers the question reaches for the button either way. What was cut: the vertex counts ("2 of the 3 points"), which the `warn`/`note` branch already conveys, and the wrong-creek alternative, which is NOT lost — a bad snap has two dedicated warnings of its own further up `renderBasinInfo()` (the `movedM` ones), so repeating it here bought nothing and doubled the length. Do not restore the second cause to this string.
- **The trace ends at the bottom of the DESCENT, not at the car.** "down to where you walk out" was read as an instruction to trace the exit hike too (field-flagged 2026-09-07), which would drag the line far outside the basin and fire the containment warning on a correct pour point. The copy now says "top to bottom — not the hike out" in all three user-facing places (button tooltip, in-draw note, README) — change them together. The walk out still climbs and still leaves the basin when someone traces it anyway, so the quiet `note` branch stays; it is just rarer than the old wording made it.
